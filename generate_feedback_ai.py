...
## 学生段落
不要把孩子每个话题都说一遍——家长没看课堂素材，罗列一堆零散发言他看不懂问题和回答的关联。改成：

**只挑 1-2 个最精彩的回答，写"完整链条"**：当时在讨论什么（一句话带出问题/情境）→ 孩子怎么答的 → 这个回答好在哪。这样家长不看素材也能看懂孩子在思考什么。

然后**收尾一句**：孩子在这节课核心主题上的收获（不是"表现很好"这种空话，是"他慢慢摸到了'喜欢'和'占有'的边界"这种具体的）。

可选：如果有值得接着聊的问题，加一句"也许可以聊聊xxx"——但别硬塞，没有就不写。

写法要求：
- 挑的是"最能体现他思考"的回答，不是发言最多的
- 每个观察后面必须跟具体内容。不要写"观察很细"就停
- 控制"很"字。一段最多两三个
- 不同学生必须有差异
- 不引原话
...
def build_student_prompt(title, student_name, speeches, history_text, gender_info=""):
    ...
    parts.append("只挑 1-2 个最精彩的回答写完整链条：讨论的问题是什么 → 他怎么答的 → 好在哪。别罗列所有话题。")
    parts.append("结尾一句写他在这节课核心主题上的收获。如果有值得接着聊的，加一句'也许可以聊聊xxx'，没有就不写。")
